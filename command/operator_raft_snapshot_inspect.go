...
func (c *OperatorRaftSnapshotInspectCommand) Run(args []string) int {
	...
	// testing...
	_, data, err := parseSnapshotNew(hclog.New(nil), file, true)
	if err != nil {
		c.UI.Error(err.Error())
		return 1
	}

	value, ok := data.Get([]byte(args[1]))
	if !ok {
		return 0
	}

	c.UI.Output(string(value.([]byte)))
...
func parseSnapshotNew(logger hclog.Logger, filename string, loadValues bool) (*raft.SnapshotMeta, *iradix.Tree, error) {
